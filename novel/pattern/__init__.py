...
class Sentiment(lazydict):
    ...
    def __call__(self, s, negation=True, **kwargs):
        ...
        return Score(polarity = avg([(w_p_s_x[0], w_p_s_x[1]) for w_p_s_x in a], weight),
                 subjectivity = avg([(w_p_s_x1[0], w_p_s_x1[2]) for w_p_s_x1 in a], weight),
                  assessments = a)
        ...
        for w, pos in words:
            ...
            if w in self and pos in self[w]:
                ...
                if w.isalpha() is False and len(w) <= 5 and w not in PUNCTUATION: # speedup
                    for (type, p), e in list(EMOTICONS.items()):
...
def language(s):
    ...
    return max(list(p.items()), key=lambda k_v: (k_v[1], int(k_v[0] == "en")))
